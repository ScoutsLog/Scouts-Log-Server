...
class eyewire_stats {
	...
	public function get_cell_summary() {
		$res = $this->db->query('SELECT c.id AS cell, c.name AS cellName, c.name_ko AS cellNameKO, c.difficulty, COUNT(t.id) AS tasks FROM cells c INNER JOIN ((SELECT c2.id FROM cells c2 WHERE c2.dataset = 1 AND c2.active = 1) UNION DISTINCT (SELECT t2.cell FROM tasks t2 WHERE t2.active = 1)) cl ON (cl.id = c.id) LEFT JOIN tasks t ON (c.id = t.cell AND t.active = 1) WHERE c.dataset = 1 GROUP BY c.id ORDER BY c.difficulty, c.name');
	...
	public function get_open_task_count() {
		$res = $this->db->query('SELECT COUNT(*) FROM tasks t INNER JOIN status s ON (t.status = s.status) INNER JOIN cells c ON (t.cell = c.id) WHERE c.dataset = 1 AND t.active = 1 AND s.include_open = 1');
	...
	public function get_open_task_summary() {
		$res = $this->db->query('select s.status, s.text AS statusText, SUM(CASE WHEN t.id IS NOT NULL AND c.dataset = 1 THEN 1 ELSE 0 END) AS tasks, s.sequence from status s left join tasks t on (t.status = s.status AND t.active = 1) left join cells c on (t.cell = c.id) where s.include_open = 1 group by s.status order by s.sequence');
